...
String backend = "https://doc-udi-backend.herokuapp.com";

Future<void> signInPost(String name, String email, String password) async {
  final String url = backend + "/api/user/create-user";
...
Future<void> login(String email, String password) async {
  // print(email);
  final String url = backend + "/api/user/sign-in";
  final response = await http.post(
    Uri.parse(url),
    body: {
      "email": email,
...
Future<void> bookAppointment(Map<String, dynamic> dataAppt) async {
  final String url = backend + "/api/user/book-appointment";
...
Future<List<dynamic>> getDoctors(String specs) async {
  final String url = backend + "/api/user/get-doc-specialization";
  final response = await http.post(
    Uri.parse(url),
    body: {
...
Future<List<dynamic>> getUpcomingAppointments(String id) async {
  final String url = backend + "/api/user/upcoming-appointment";
  print("ID: " + id);
  final response = await http.post(
    Uri.parse(url),
    body: {
...
Future<List<dynamic>> getPreviousAppointments(String id) async {
  final String url = backend + "/api/user/previous-appointment";
  final response = await http.post(
    Uri.parse(url),
    body: {
      "patientID": id,
...
Future<void> startAppointment(Map<String, dynamic> map) async {
  final String url = backend + "/api/user/start-appointment";
...
Future<bool> checkActive(Map<String, dynamic> map) async {
  final String url = backend + "/api/user/end-appointment";
  var postData = json.encode(map);
  final response = await http.post(
    Uri.parse(url),
    body: postData,
...
Future<void> uploadPhoto(String purl, String id) async {
  final String url = backend + "/api/user/update-pfp";
  final response = await http.post(
    Uri.parse(url),
    body: {
      "id": id,
